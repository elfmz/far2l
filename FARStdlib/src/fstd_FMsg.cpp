...
int WINAPI FP_Message(unsigned int Flags, LPCSTR HelpTopic, LPCSTR *Items, int ItemsNumber, int ButtonsNumber,
		LPBOOL Delayed /*NULL*/)
{
	LPCSTR litems[100];
	int rc;
	...
	rc = FP_Info->Message(FP_Info->ModuleNumber, Flags, HelpTopic,
			IS_FLAG(Flags, FMSG_ALLINONE) ? Items : litems, ItemsNumber, ButtonsNumber);
