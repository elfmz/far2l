...
void AWSFileWriter::StartMultipartUpload() {
    ...
    if (!outcome.IsSuccess()) {
        throw std::runtime_error("Failed to start multipart upload: " + outcome.GetError().GetMessage());
    }
...
void AWSFileWriter::UploadPart() {
    ...
    if (!outcome.IsSuccess()) {
        throw std::runtime_error("Failed to upload part: " + outcome.GetError().GetMessage());
    }
...
void AWSFileWriter::CompleteMultipartUpload() {
    if (!buffer.empty()) {
        UploadPart(); // Загружаем оставшиеся данные
    }
    ...
    if (!outcome.IsSuccess()) {
        throw std::runtime_error("Failed to complete multipart upload: " + outcome.GetError().GetMessage());
    }
}
