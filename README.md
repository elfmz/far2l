...
#### Or simply on Ubuntu:
``` sh
apt-get install gawk m4 libglib2.0-dev libwxgtk3.0-dev libssh-dev libssl-dev libsmbclient-dev libnfs-dev libneon27-dev libarchive-dev cmake g++ git
```
...
```sh
brew install cmake gawk glib ninja pkg-config wget wxmac libssh libnfs neon libarchive
```
