...
const RenderLayout &ResolveLayout() const
	...
	void DrawCell(int col, int row, const CHAR_INFO &ci)
    {
		...
		const SDL_BlendMode blend_mode = glyph->is_color ? SDL_BLENDMODE_BLEND : ComposePremultBlendMode();
	...
	Glyph *GetGlyph(const std::u32string &cluster)
	{
		...
		const bool use_bitmap_fallback = _bitmap_fallback || bitmap.width > kGlyphAtlasWidthPx || bitmap.height > kGlyphAtlasHeightPx;
