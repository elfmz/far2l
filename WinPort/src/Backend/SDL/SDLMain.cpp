...
void SDLConsoleBackend::HandleHostResize(int pixel_w, int pixel_h)
{
	...
	const bool same_px = (pixel_w == _last_host_px_w && pixel_h == _last_host_px_h);
	_last_host_px_w = pixel_w;
	_last_host_px_h = pixel_h;
	int output_w = 0;
	int output_h = 0;
	if (_window) {
		SDL_GetWindowSize(_window, &output_w, &output_h);
	}
	int out_px_w = 0;
	int out_px_h = 0;
	...
	if (SDL_TRACE_ENABLED()) {
		SDL_TRACE_EVENT("console", "host_resize", "px=%d,%d cell=%d,%d",
			pixel_w, pixel_h, _renderer.CellWidth(), _renderer.CellHeight());
	}
	const int size_w = out_px_w > 0 ? out_px_w : pixel_w;
	const int size_h = out_px_h > 0 ? out_px_h : pixel_h;
