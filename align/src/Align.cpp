...
#ifndef UNICODE
	int ExitCode = Info.Dialog(Info.ModuleNumber, -1, -1, 76, 10, NULL, DialogItems, ARRAYSIZE(DialogItems));
#else
...
void ReformatBlock(int RightMargin, int SmartMode, int Justify)
{
	...
	for (int I = LastSplitPos; I < TotalLength; I++) {
		...
		if (TotalString[I] == _T(' ') && Length > RightMargin - IndentSize
				&& LastLength <= RightMargin - IndentSize) {
			if (LastLength <= 0) {
				PrevSpacePos = I;
				LastLength = Length;
			}
			...
			while (ess.StringLength > 0 && ess.StringText[ess.StringLength - 1] == _T(' '))
				ess.StringLength--;
			if (!Justify || ess.StringLength >= RightMargin - IndentSize
					|| !JustifyString(RightMargin - IndentSize, ess))
...
int JustifyString(int RightMargin, struct EditorSetString &ess)
{
	...
	for (I = 0; I < RightMargin - 1; I++)
		if (NewString[I] != _T(' ') && NewString[I + 1] == _T(' ')) {
			...
			if (MoveSize == 0)
				break;
			memmove(NewString + I + 1 + MoveSize, NewString + I + 1,
					(RightMargin - (I + 1 + MoveSize)) * sizeof(TCHAR));
			while (MoveSize--)
