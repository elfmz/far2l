...
UInt64 parse_size_string(const std::wstring &str)
{
	...
	if (i < str.size()) {
		...
		UInt64 mod_mul;
		if (mod_ch == L'K' || mod_ch == L'k')
			mod_mul = 1024;
		else if (mod_ch == L'M' || mod_ch == L'm')
			mod_mul = 1024 * 1024;
		else if (mod_ch == L'G' || mod_ch == L'g')
			mod_mul = 1024 * 1024 * 1024;
		else
			mod_mul = 1;
		size *= mod_mul;
	}
...
std::wstring load_file(const std::wstring &file_path, unsigned *code_page)
{
	...
	} else if (size >= 3 && buffer.data()[0] == '\xEF' && buffer.data()[1] == '\xBB'
			&& buffer.data()[2] == '\xBF') {
		if (code_page)
			*code_page = CP_UTF8;
