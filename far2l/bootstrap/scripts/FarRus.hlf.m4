...
$ #Особенности FAR2L - начало работы#
 ...
 #Особенности под Wayland или под WSL+WSLg (при проблемах с буфером обмена и/или комбинациями клавиш в FAR2L-GUI/TTYX)#
    Для адекватной работы в Wayland помогает запустить в режиме #xWayland#, указав переменную среды #GDK_BACKEND=x11#, т.к. FAR2L (режимы GUI и TTYX) для работы с буфером обмена и получения расширенных сочетаний клавиш использует возможности X11, которые могут быть не полностью совместимы в чистом Wayland'е:
 ...
 #Командная строка FAR2L & bash#
    ...
    Если по умолчанию ваша системная оболочка не bash, возможно, вам будет удобно также прописать ваши переменные среды в файлах запуска bash.
...
изначально лицензированны под 3-clause BSD лицензией.
Смотрите LICENSE.txt и LICENSE.Far2.txt в исходных файлах для деталей.

@CmdLine
$ # FAR2L: параметры командной строки#
  Вы можете задавать следующие аргументы в командной строке или переменной окружения FAR2L_ARGS:
...
случае отсутствия таких расширений терминала - far2l проверяет наличия доступа к X11 сесии и
использует ее для улучшения возможностей интерфейса, если был собрать с опцией TTYX.
...
расширений и разрешает к использованию лишь базовые возможности обычного терминала в TTY режиме.
  ...
  #--mortal#
  Этот аргумент применим только для far2l, который работает в режиме TTY. По умолчанию пр закрытии
...
командной строки полночтью отключает эту функциональность, так что закрытие терминала будет приводить
...
"~Путь к персональным плагинам~@PluginsManagerSettings@"
  ...
  #/m#
  При старте FAR2L не будет загружать макрокоманды из реестра.
  ...
  FAR2L установит ~переменную среды~@FAREnv@ "FARUSER" в значение, равное <username>.
...
в указанном примере), то FAR2L будет ждать завершения входного потока вечно.
Это особенность текущей версии FAR2L.
...
команда для пассивной панели (пассивная панель при этом временно активируется). Односимвольные префиксы игнорируются.
  Пример: far ma:c:\\Far20.7z "macro:post MsgBox(\\"FAR2L\\",\\"Successfully started\\")"
...
не-управляющей (цифро-символьной) кнопкию:
...
Если же вы хотите закрыть far2l с командной строки: напечатайте псевдокоманду 'exit far' - она будет распознана far2l как команда закрытия.
...
вызвать повреждение или не сохранение данных убитого приложения. Если far2l работает в TTY то вы так же можете использовать сочетание #Ctrl+Alt+Z# чтобы поместить far2l в фон, освободив терминал, но продолжая исполнять текущую команду.
...
то FAR2L пытается использовать ассоциации Windows для запуска этого типа
файлов.
...
в ~ассоциациях файлов~@FileAssoc@ FAR2L, будет запущена программа Windows, предназначенная для обработки этого типа файлов.

  #Автоматически монтировать CDROM#
  При выборе компакт-диска в меню ~выбора дисков~@DriveDlg@ FAR2L задвинет открытый лоток привода компакт-дисков. Отключите эту опцию,
...
пути: #~~/.config/far2l/favorites# этот файл должен содержаь строки, каждая из которых
...
вывод будет вставлен в содержимое favories файла при построении меню. Это позволяет
