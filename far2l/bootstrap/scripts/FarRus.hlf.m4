...
запуска команд на удаленном SSH сервере. Примером такого плагина является NetRocks, а точнее поддерживаемые им SFTP/SCP протоколы.
  ...
  Очистка истории терминала и экрана                                #F8#
    (! будьте осторожны, очистка сразу без дополнительного подтверждения !)
...
метасимволы:
    ...
    #!+!#      Аналогично !-!, но если длинное имя файла утеряно
             после выполнения команды, FAR2L восстановит его
...
UNIX режимы доступа как у оригинальных файлов, за исключением битов отфильтрованных umask.
    ...
    Если опция #Копировать расширенные а&ттрибуты# включена, то копии файлов получат такие же
расширенные аттрибуты как у оригинальных файлов.
...
или же его первоначальная копия будут перезаписаны.
    ...
    Выпадающий список #Символические сс&ылки# позволяет выбрать один мз трех возможных 
...
он указывает на на 'внешний' файл или один из так же копируемых файлов. В первом случае
