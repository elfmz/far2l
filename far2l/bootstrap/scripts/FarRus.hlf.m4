...
вновь созданную папку #d:\folder2\#.
  ...
  Запуск от имени администратора                      #Ctrl-Alt-Enter#
    ...
    #Shift-Enter# для папки вызывает графический файловый проводник с показом
содержимого выбранной папки. Для того, чтобы в проводнике показать корневую папку,
нужно нажать #Shift-Enter# на требуемой строке в меню ~перехода~@DriveDlg@.
...
'Plugins', которая находится в одной папке с FAR2L.EXE. При обнаружении нового
модуля FAR2L сохраняет информацию о нём и впоследствии загружает его только при
...
над папками.
	...
	!! Не актуально для Linux !!

    FAR2L запоминает информацию о структуре папок в файле #tree2.far#,
...
папке, что и FAR2L.EXE.
    По умолчанию файл tree2.far отсутствует, для его создания нужно хотя бы один
...
переименование, копирование) FAR2L отслеживает и обновляет его
внутреннюю структуру. Если изменения были сделаны не в FAR2L обновить
...
команды "Файлы описания папок" в ~Меню параметров~@OptMenu@.
    ...
    Для приводов компакт-дисков FAR2L умеет определять тип привода. Различаются
...
комбинация клавиш #Ctrl-PgUp# аналогична клавише #Esc# - закрыть меню перехода.
    ...
    #Shift-Enter# вызывает графический файловый проводник с показом выбранного пути.
