...
~Настройки редактора~@EditorSettings@.
  ...
  Прочие операции
   ...
   #Ctrl-Shift-F5#           Переключить отображение физических окончаний строк
...
оставляет текущую позицию без изменений.
    ...
    Когда показ окончаний строк включён клавишами #Ctrl-Shift-F5#, физические
окончания строк отображаются как #␍# (CR) и #␊# (LF), что позволяет увидеть
...
внешнего и ~встроенного редакторов~@Editor@.
  ...
  #Окончания строк#         Показывать физические окончания строк как #␍# (CR)
                          и #␊# (LF). Опция переключается нажатием
                          #Ctrl-Shift-F5# в редакторе.
...
целиком. #$*# заменяется на символ '*'.
    ...
    #\r# и #\n# обозначают буквальные символы CR и LF. Захваченные окончания строк
также сохраняются точно, поэтому #\r\n# можно заменить на #\n#, а каждый символ
пары CRLF можно искать и заменять отдельно.
