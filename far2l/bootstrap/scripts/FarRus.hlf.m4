...
$ #Меню команд#
   ...
   #Меню пользователя#    Позволяет редактировать главное или местное
                        ...
                        используется #Ins# или #Ctrl+N#, для удаления - #Del#,
...
#program "!.!"#, а не #program !.!#.
    ...
    'B' - собрать помеченные файлы с обеих панелей в один список.

    ^<wrap>При использовании модификатора #B# временный файл будет содержать
помеченные (выделенные) файлы как с активной, так и с пассивной панели.
Команда выполняется один раз для всех выделенных файлов, а не по одному разу
для каждого файла. Удобно для сравнения файлов с разных панелей.
...
заключены в кавычки.
    ...
    Ассоциация #!@@BF!# - имя файла, содержащего список помеченных файлов с
обеих панелей с полным путём. Пример использования в пользовательском меню:

      diff "$(sed -n '1p' "!@@BF!")" "$(sed -n '2p' "!@@BF!")"

    ^<wrap>Пометьте один файл на левой панели и один на правой панели (клавишей #Insert#),
затем выполните команду для их сравнения.
