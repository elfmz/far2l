...
обеспечивает обработку файлов с #длинными именами# и имеет обширный набор
...
$ # FAR2L: параметры командной строки#
  ...
  #--tty#
  Запускает far2l сразу в режиме TTY вместо автоматического определния возможности
...
панелями возможно нажатием сочетания Ctrl+O или щелчком по левому верхнему углу окна.
    #Расширения терминала FAR2L# хотя FAR2L сам по себе это приложение ткстового режима, оно может работать в режиме вывода через подсистему GUI
или TTY (терминал). Хотя TTY режим может функционировать в обычном терминале, как xterm или в терминале клиента SSH сессии, но он так же может
работать внутри терминала другого far2l, получая от этого дополнительные возможности, недоступные под 'обычными' терминалами, такие как
распознавание всех кнопок клавиатуры и событий нажатия/отпускания кнопок. Так же 'хостовый' far2l может предоставлять доступ к буферу обмена хоста
и механизм уведомлений рабочего стола. Вы можете использовать эту функциональность запуская TTY far2l на SSH сервере внутри SSH сессии клиента, запущенного
внутри терминала far2l, работающего в GUI режиме на хосте клиента. Или, что намного проще, используя плагин с возможностью запуска команд на удаленном
SSH сервере. Примером такого плагина является NetRocks, а точнее поддерживаемые им SFTP/SCP протоколы.
...
только для дисков).
    ...
    Если включён показ параметров компакт-дисков (#Ctrl-8#), то FAR2L будет
