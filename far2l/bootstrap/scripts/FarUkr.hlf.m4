...
2. ^<wrap>Для асоціацій ! @ @! та !$! допускаються такі модифікатори:
 ...
 'B' – зібрати позначені файли з обох панелей в один список.

    ^<wrap>При використанні модифікатора #B# тимчасовий файл міститиме
позначені (виділені) файли як з активної, так і з пасивної панелі.
Команда виконується один раз для всіх виділених файлів, а не по одному разу
для кожного файлу. Зручно для порівняння файлів з різних панелей.
...
укладені в лапки.
    ...
    Асоціація #!@@BF!# - ім'я файлу, що містить список позначених файлів з
обох панелей з повним шляхом. Приклад використання в користувацькому меню:

      diff "$(sed -n '1p' "!@@BF!")" "$(sed -n '2p' "!@@BF!")"

    ^<wrap>Позначте один файл на лівій панелі та один на правій панелі (клавішею #Insert#),
потім виконайте команду для їх порівняння.
