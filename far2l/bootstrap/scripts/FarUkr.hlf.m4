...
запуску FAR2L із telnet.
 ...
 #/ag#
 Заборона відображення псевдографічних символів.
...
#Alt-Shift-F9# - відкрити меню ~ "Конфігурація модулів" ~ @ Plugins Config @.
...
- ~відключення мережевого пристрою ~@DisconnectDrive @ з меню дисків;
 ...
 - відключення SUBST-диска з меню дисків;
...
або меню вибору дисків (якщо плагін Network відсутня).
 ...
 Додаткова інформація, що відображається у заголовку вікна консолі.
Може містити будь-який текст, включаючи такі змінні:
 - #%Ver# - версія FAR2L;
...
#Alt-Shift-F9# Виклик діалогу ~налаштування програми перегляду ~@ViewerSettings @
...
#Del# ^<wrap>Видалити символ (також може видалити блок, залежно від ~Налаштувань редактора ~@EditorSettings @)
 ...
 #Ctrl-Shift-Клав.курсора# Позначка блоку
 #Alt-сірі клав.курсора# Позначка вертикального блоку
 #Alt-Shift-Клав.курсора# Позначка вертикального блоку
 #Ctrl-Alt-сірі кнопки# Позначка вертикального блоку
...
Клавіші #Ctrl-A#, #F4# викликають діалог ~ файлових атрибутів ~ @ FileAttrDlg @.
...
запитує привілеї суперкористувача.
 ...
 #Ctrl-1# - #Ctrl-9# переключають відображення різної інформації:

 Ctrl-1 – тип диска;
 Ctrl-2 - мережеве ім'я/шлях, асоційований з SUBST-диском/ім'я файлу, що містить віртуальний диск;
 Ctrl-3 – мітка диска;
 Ctrl-4 – файлова система;
 Ctrl-5 - загальне та вільне місце на диску
 (Ця опція має два режими, натисніть двічі щоб побачити);
 Ctrl-6 – показ параметрів змінних дисків;
 Ctrl-7 - показ імен модулів, що підключаються;
 Ctrl-8 – показ параметрів компакт-дисків;
 Ctrl-9 – показ параметрів мережних дисків.

 Налаштування меню переходу зберігаються у конфігурації FAR2L.

 #F9# показує діалог для налаштування відображення цієї інформації.
...
лише для дисків).
 ...
 Якщо увімкнено показ параметрів компакт-дисків (#Ctrl-8#), то FAR2L буде
намагатись визначити тип CD-приводу. Розрізняються такі приводи: CD-ROM,
CD-RW, CD-RW/DVD, DVD-ROM, DVD-RW та DVD-RAM. Функція визначення типу приводу
вимагає або адміністративних прав, або дозволу доступу до локальних CD-приводів
за допомогою політик (для цього треба запустити з командного рядка утиліту #secpol.msc#
та встановити налаштування '#Локальні політики/Параметри безпеки/Пристрої:#
#Дозволити доступ до дисків компакт-дисків тільки локальним користувачам#'
у значення '#Дозволити#').
...
модулю контекстно-залежну підказку (якщо плагін має файл допомоги).
 ...
 Символ #A# у крайній лівій колонці меню означає, що відповідний плагін
розроблений для Far 1.7x і не підтримує всіх можливостей Far 2 (зокрема,
символів Unicode в іменах файлів та у редакторі).
