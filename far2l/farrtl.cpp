...
extern "C"
{
	_CRTIMP int64_t __cdecl _ftelli64(FILE *str);
	_CRTIMP int __cdecl _fseeki64(FILE *stream, int64_t offset, int whence);
};
#endif
...
)
{
	...
	for (;;)      /* exit in middle of loop */
	{
		...
		if (number < maxval || (number == maxval &&
		                        (uint64_t)digval <= _UI64_MAX % ibase))
		{
...
int64_t _cdecl _strtoi64(const char *nptr,char **endptr,int ibase)
{
	return (int64_t) strtoxq(nptr, (const char **)endptr, ibase, 0);
}
#endif
#endif
...
#if 0
static uint64_t __cdecl wcstoxq(
    const wchar_t *nptr,
...
#ifndef _MSC_VER  // overload in v8
int64_t __cdecl _wcstoi64(const wchar_t *nptr,wchar_t **endptr,int ibase)
{
	return (int64_t) wcstoxq(nptr, (const wchar_t **)endptr, ibase, 0);
}

uint64_t __cdecl _wcstoui64(const wchar_t *nptr,wchar_t **endptr,int ibase)
{
