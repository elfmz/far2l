			...
			case DN_BTNCLICK:
				switch (Param1) {
					...
					case ME_BUTTON_MLIB_TO_SEQUENCE: {
							...
							COORD coord{0,0};
							SendDlgMessage(hDlg, DM_GETCURSORPOS, ME_MEMOEDIT_SEQUENCE, reinterpret_cast<LONG_PTR>(&coord) );

#if 0
							EditorSelect es;
							SendDlgMessage(hDlg, DM_GETSELECTION, ME_MEMOEDIT_SEQUENCE, reinterpret_cast<LONG_PTR>(&es) );

							{ // DEBUG
								ExMessager em(L"Macro Browser - Edit Debug");
								em.AddFormat(L"Sequence Field coord: X=%d Y=%d", coord.X, coord.Y);
								em.AddFormat(L"Sequence Field selection:");
								em.AddFormat(L" BlockType=%d (%s), BlockStartLine=%d, BlockStartPos=%d,",
									es.BlockType,
									(es.BlockType == BTYPE_NONE ? "NONE"
										: (es.BlockType == BTYPE_STREAM ? "STREAM"
											: (es.BlockType == BTYPE_COLUMN ? "COLUMN" : "????"))),
									es.BlockStartLine, es.BlockStartPos);
								em.AddFormat(L" BlockWidth=%d, BlockHeight=%d", es.BlockWidth, es.BlockHeight);
								em.AddDup(L"strSeq:");
								em.AddMultiline(strSeq);
								em.Add(L"Continue");
								em.Show(MSG_LEFTALIGN, 1);
							}
#endif

							// search position for string coord.Y
							size_t ipos;
							int in;
							for( ipos = 0, in = 0; in < coord.Y; ipos++, in++) {
								if(!strSeq.Pos(ipos, '\n', ipos)) {
									ipos = strSeq.GetLength();
									break;
								}
							}
							// insert in start of string coord.Y
							strSeq.Insert(ipos, L" ");
							strSeq.Insert(ipos, List.Item.Text);
							strSeq.Insert(ipos, L" ");
							...
							SendDlgMessage(hDlg, DM_SETCURSORPOS, ME_MEMOEDIT_SEQUENCE, reinterpret_cast<LONG_PTR>(&coord) );
				...
				if (Param1 == ME_BUTTON_OK) {
					...
					{ // DEBUG
							...
							em.AddFormat(L" BlockType=%d (%s), BlockStartLine=%d, BlockStartPos=%d",
								...
								es.BlockStartLine, es.BlockStartPos);
							em.AddFormat(L" BlockWidth=%d, BlockHeight=%d", es.BlockWidth, es.BlockHeight);
							em.AddDup(L"\x2 pstrSequence:");
							em.AddMultiline(pstrSequence);
							em.AddDup(L"\x2 pstrDescription:");
							em.AddDup(pstrDescription);
							em.Add(L"Continue");
							em.Show(MSG_LEFTALIGN, 1);
					}
#endif
...
bool MacroBrowser::Edit(int imacro)
{
	...
	*/
	...
	const int x_mlib = (int)(MacroLib_KeywordsFunctions2Items(ComboListMLib_Items) + 3);
...
void MacroBrowser::PrepareVMenu()
{
	...
	mi.strName.Format(L" Macros: all=%d, (+) enabled=%d, (-) disabled=%d, (NS) need save=%d, (D) marked as deleted=%d",
		Macro->MacroLIBCount, stat_macros_enabled, stat_macros_disabled, stat_need_save, stat_macros_deleted);
	mi.Flags = 0;
	ListMacro.AddItem(&mi);
	fs2copy += "\n" + mi.strName;
	v_menu_macroindex.emplace_back(-1, -1);
