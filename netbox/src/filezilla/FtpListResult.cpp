...
bool CFtpListResult::ParseShortDate(const char *str, int len, t_directory::t_direntry::t_date &date) const
{
  ...
  if (!numeric)
  {
    ...
    USES_CONVERSION;
    std::map<CString, int>::const_iterator iter = const_cast<CFtpListResult *>(this)->m_MonthNamesMap.find(A2T(tmpstr));
    nb_free(tmpstr);
...
BOOL CFtpListResult::parseAsVMS(const char *line, const int linelen, t_directory::t_direntry &direntry)
{
  ...
  strlwr(buffer);
  std::map<CString, int>::const_iterator iter = m_MonthNamesMap.find(A2T(buffer));
  if (iter == m_MonthNamesMap.end())
    return FALSE;
...
BOOL CFtpListResult::parseAsUnix(const char *line, const int linelen, t_directory::t_direntry &direntry)
{
  ...
  while (str && !ParseSize(str, tokenlen, tmp) && !IsNumeric(skipped, skippedlen))
  {
    ...
    USES_CONVERSION;
    std::map<CString, int>::const_iterator iter = m_MonthNamesMap.find(A2T(tmpstr));
    nb_free(tmpstr);
    if (iter != m_MonthNamesMap.end())
    {
  ...
  else
  {
    ...
    USES_CONVERSION;
    std::map<CString, int>::const_iterator iter = m_MonthNamesMap.find(A2T(lwr));
    nb_free(lwr);
    if (iter == m_MonthNamesMap.end())
    {
      int i;
