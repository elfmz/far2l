...
if command -v nix &>/dev/null; then
    ...
    # Update only far2l version (not _7z-far)
    # This matches the far2l block specifically
    sed -i '/^[[:space:]]*far2l = /,/^[[:space:]]*};/ {
        s/^\([[:space:]]*\)version = ".*";/\1version = "'${NEW_VERSION}'";/
    }' ./NixOS/far2lOverlays.nix
    
    echo "Updated far2l version to $NEW_VERSION"
else
