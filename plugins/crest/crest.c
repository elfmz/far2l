...
#define CR_VERSION        _T("Crest/2l v1.12")
...
static void Ruler()
{
    ...
    for( i=0; i<EInfo.WindowSizeX; i+=10 ){
        int col = (i?i:1)+EInfo.LeftPos;
        _tstrcpy(buff,sz4dot);
        buff[apiSnprintf(buff, sizeof(buff)/sizeof(buff[0]), szDecimalFmt, col)]=_T('.');
        if ((col+10) < EInfo.CurPos || (col-1) > EInfo.CurPos) {
            Info.Text(i, 0, Options.RulerColor, buff);
        } else {
            TCHAR hot_char[2];
            int highlight_pos = EInfo.CurPos-col+(i?0:1);
            hot_char[0] = buff[highlight_pos];
            hot_char[1] = _T('\0');
            buff[highlight_pos] = 0;
            if (highlight_pos)
                Info.Text(i, 0, Options.RulerColor, buff);
            Info.Text(i+highlight_pos, 0, Options.CenterColor, hot_char);
            if (highlight_pos < 10)
                Info.Text(i+highlight_pos+1, 0, Options.RulerColor, buff+highlight_pos+1);
        }
    }
}
...
SHAREDSYMBOL int WINAPI EXP_NAME(ProcessEditorInput)( const INPUT_RECORD * ir)
{
  ...
  //Force repaint if changes
    if ( last.isHilight != last.hilight ) {
        ...
        if( last.isHilight && isShowRuler() && !(EInfo.Options & EOPT_MEMOEDIT) )
            Ruler();
    }
