...
typedef void ( *FARSTDQSORT)(void *base, size_t nelem, size_t width, int ( *fcmp)(void *, void *));
typedef void ( *FARSTDQSORTEX)(void *base, size_t nelem, size_t width, int ( *fcmp)(void *, void *,void *userparam),void *userparam);
typedef void *( *FARSTDBSEARCH)(void *key, void *base, size_t nelem, size_t width, int ( *fcmp)(void *, void *));
typedef int ( *FARSTDGETFILEOWNER)(wchar_t *Computer,wchar_t *Name,wchar_t *Owner,int Size);
typedef int ( *FARSTDGETNUMBEROFLINKS)(wchar_t *Name);
typedef int ( *FARSTDATOI)(wchar_t *s);
typedef int64_t ( *FARSTDATOI64)(wchar_t *s);
...
typedef wchar_t*( *FARSTDPOINTTONAME)(wchar_t *Path);
typedef int ( *FARSTDGETPATHROOT)(wchar_t *Path,wchar_t *Root, int DestSize);
typedef BOOL ( *FARSTDADDENDSLASH)(wchar_t *Path);
typedef int ( *FARSTDCOPYTOCLIPBOARD)(wchar_t *Data);
typedef wchar_t *( *FARSTDPASTEFROMCLIPBOARD)(void);
typedef int ( *FARSTDINPUTRECORDTOKEY)(INPUT_RECORD *r);
...
typedef int ( *FARSTDLOCALSTRICMP)(wchar_t *s1,wchar_t *s2);
typedef int ( *FARSTDLOCALSTRNICMP)(wchar_t *s1,wchar_t *s2,int n);
...
typedef int ( *FARSTDPROCESSNAME)(wchar_t *param1, wchar_t *param2, DWORD size, DWORD flags);
...
typedef int ( *FARSTDKEYNAMETOKEY)(wchar_t *Name);

typedef int ( *FRSUSERFUNC)(
    struct FAR_FIND_DATA *FData,
    wchar_t *FullName,
    void *Param
);
...
typedef void ( *FARSTDRECURSIVESEARCH)(wchar_t *InitDir,wchar_t *Mask,FRSUSERFUNC Func,DWORD Flags,void *Param);
typedef int ( *FARSTDMKTEMP)(wchar_t *Dest, DWORD size, wchar_t *Prefix);
...
typedef int ( *FARSTDMKLINK)(wchar_t *Src,wchar_t *Dest,DWORD Flags);
typedef int ( *FARGETREPARSEPOINTINFO)(wchar_t *Src, wchar_t *Dest,int DestSize);
...
typedef int ( *FARCONVERTPATH)(enum CONVERTPATHMODES Mode, wchar_t *Src, wchar_t *Dest, int DestSize);
...
typedef int ( *FAREXECUTE)(wchar_t *CmdStr, unsigned int ExecFlags);
typedef int ( *FAREXECUTE_LIBRARY)(wchar_t *Library, wchar_t *Symbol, wchar_t *CmdStr, unsigned int ExecFlags);
...
struct PluginInfo
{
 int StructSize;
 DWORD Flags;
 wchar_t * *DiskMenuStrings;
 int *Reserved0;
 int DiskMenuStringsNumber;
 wchar_t * *PluginMenuStrings;
 int PluginMenuStringsNumber;
 wchar_t * *PluginConfigStrings;
...
class Plugin:
    ...
    def SetStartupInfo(self, Info):
        ...
        for name in dir(self.Info):
            print('Info.%s='%name, getattr(self.Info, name))

    def GetPluginInfo(self, Info):
        Info = ffi.cast("struct PluginInfo *", Info)
        self._MenuItems = [
            ffi.new("wchar_t []", "Python"),
        ]
        self.MenuItems = ffi.new("wchar_t *[]", self._MenuItems)
        Info.Flags = ffic.PF_EDITOR|ffic.PF_DISABLEPANELS
    ...
    def OpenPlugin(self, OpenFrom, Item):
        _MsgItems = [
            ffi.new("wchar_t []", "Python"),
            ffi.new("wchar_t []", ""),
            ffi.new("wchar_t []", "python.cpp: compiling..."),
            ffi.new("wchar_t []", "   0 error(s), 0 warning(s) :-)"),
            ffi.new("wchar_t []", ""),
            ffi.new("wchar_t []", "\x01"),
            ffi.new("wchar_t []", "&Ok"),
        ]
        ...
        self.Info.Message(
            ...
            ffi.new("wchar_t []", "Contents"),          # HelpTopic
            MsgItems,                                   # Items
