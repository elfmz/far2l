...
class Entry(object):
    def __init__(self, dirname=None, perms=None, links=0, uid=0, gid=0, size=None, date=None, name=None, date_re=None):
        self.mode = 0
        self.perms = perms
    ...
    def __str__(self):
        ...
        template = ('{mode:5o} {perms} {links:>4} {uid:<8} {gid:<8} {size:>8} {date} {name}')
...
class Docker(object):
    ...
    def ls(self, deviceid, top):
        ...
        lines = self.run('exec', deviceid, '/bin/ls', '-anL', '--time-style=long-iso', top)
        result = []
        for line in lines:
            line = line.strip()
            if not line:
