...
class Plugin(PluginBase):
    ...
    def OpenPlugin(self, OpenFrom):
        if OpenFrom == 5:
            ...
            return
        try:
            img = Image.open(fqname)
        except:
            log.debug(f"unsupported image type fqname=%s", OpenFrom, fqname)
            return
        try:
            self.View(fqname, img)
        finally:
            return

    def View(self, name, img):
        ...
        while True:
            ...
            log.debug("canvas W:%d H:%d nb=%d", canvas_w, canvas_h, len(rgb_img_bytes))
