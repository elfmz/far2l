...
void ReadFileLines(int fd, DWORD FileSizeLow, TCHAR **argv, TCHAR *args,
  ...
  if(hFile != INVALID_HANDLE_VALUE)
  {
    ...
    if(fd!=-1 && FileSizeLow!=INVALID_FILE_SIZE)
    {
      ...
      *argv = (TCHAR**)malloc(*argc*sizeof(TCHAR*) + i*sizeof(TCHAR));
      ReadFileLines(fd, FileSizeLow, *argv, (TCHAR*)&(*argv)[*argc],
...
SHAREDSYMBOL int WINAPI EXP_NAME(PutFiles)(HANDLE hPlugin,struct PluginPanelItem *PanelItem,
...
SHAREDSYMBOL int WINAPI EXP_NAME(SetFindList)(HANDLE hPlugin,const struct PluginPanelItem *PanelItem,int ItemsNumber)
{
  TmpPanel *Panel=(TmpPanel *)hPlugin;
